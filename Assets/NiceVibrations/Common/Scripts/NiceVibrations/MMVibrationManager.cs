	...
	public static class MMVibrationManager
	{
                                            ...
                                            MonoBehaviour coroutineSupport = null)
        {
            ...
            else if (iOS() && vibrateiOS)
            {
                if ((iOSVersion >= 13) && HapticsSupported())
                {
                    MMNViOSCoreHaptics.PlayTransientHapticPattern(iOSIntensity, iOSSharpness);
                    _hapticsPlayedOnce = true;
                }
                else
                {
        ...
        public static void ContinuousHaptic(float intensity, float sharpness, float duration, 
            HapticTypes fallbackOldiOS = HapticTypes.None, MonoBehaviour mono = null, bool alsoRumble = false, int controllerID = -1, bool oldiOSRegularVibrate = false)
        {
            ContinuousHaptic(true, intensity, sharpness, fallbackOldiOS, true, intensity, sharpness, false, alsoRumble, intensity, sharpness, controllerID, duration, mono, oldiOSRegularVibrate);
        }
        ...
        public static void ContinuousHaptic(bool vibrateiOS, float iOSIntensity, float iOSSharpness, HapticTypes fallbackOldiOS,
                                            ...
                                            float duration, 
                                            MonoBehaviour mono = null, bool oldiOSRegularVibrate = false)
        {
            if (!_vibrationsActive)
            {
                return;
            ...
            else if (iOS() && vibrateiOS)
            {
                ...
                {
                    MMNViOS.iOSTriggerHaptics(fallbackOldiOS, oldiOSRegularVibrate);
                }
            }
            if (rumble && (mono != null))
            {
        ...
        {
            UpdateContinuousHaptic(true, intensity, sharpness, true, intensity, sharpness, alsoRumble, intensity, sharpness, controllerID);
        }
                                                  ...
                                                  bool rumble, float rumbleIntensity, float rumbleSharpness, int controllerID = -1)
        {
            if (iOS() && ios)
            {
                if ((iOSVersion >= 13) && HapticsSupported())
                {
                    MMNViOSCoreHaptics.UpdateContinuousHapticPatternRational(iosIntensity, iosSharpness);
                    _hapticsPlayedOnce = true;
                }
            }
            if (rumble)
        ...
        public static void AdvancedHapticPattern(string iOSJSONString,
                                                ...
                                                MonoBehaviour coroutineSupport = null, int controllerID = -1, bool oldiOSRegularVibrate = false)
        {
            AdvancedHapticPattern(true, iOSJSONString, true, androidPattern, androidAmplitudes, androidRepeat, false, true, rumblePattern,
                rumbleLowFreqAmplitudes, rumbleHighFreqAmplitudes, rumbleRepeat, fallbackOldiOS, coroutineSupport, controllerID, oldiOSRegularVibrate);
        }
