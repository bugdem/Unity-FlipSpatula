...
namespace ClocknestGames.Library.Utils
{
    ...
    public static class BezierUtility
    {
        ...
        public static Bounds CalculateSegmentBounds(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3)
        {
            ...
            {
                minMax.AddValue(BezierUtility.EvaluateCurve(p0, p1, p2, p3, t));
            }
        ...
        /// <summary>
        /// Splits bezier path evenly spaced points.
        /// </summary>
        /// <param name="bezierPath"></param>
        /// <param name="spacing"></param>
        /// <param name="accuracy"></param>
        /// <returns></returns>
        public static PathSplitData SplitBezierPathEvenly(Vector3[] bezierPath, float spacing, float accuracy)
        {
            PathSplitData splitData = new PathSplitData();

            splitData.vertices.Add(bezierPath[0]);
            splitData.tangents.Add(EvaluateCurveDerivative(bezierPath, 0).normalized);
            ...
            float estimatedSegmentLength = EstimateCurveLength(bezierPath[0], bezierPath[1], bezierPath[2], bezierPath[3]);
            int divisions = Mathf.CeilToInt(estimatedSegmentLength * accuracy);
            float increment = 1f / divisions;

            for (float t = increment; t <= 1; t += increment)
            {
                bool isLastPointOnPath = (t + increment > 1);
                if (isLastPointOnPath)
                {
                    t = 1;
                }
                Vector3 pointOnPath = EvaluateCurve(bezierPath, t);
                ...
                if (dstSinceLastVertex >= spacing || isLastPointOnPath)
                {
                    ...
                    splitData.tangents.Add(EvaluateCurveDerivative(bezierPath, t).normalized);
